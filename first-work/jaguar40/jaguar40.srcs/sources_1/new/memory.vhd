...
entity memory is
    Port(
        ...
        data_mem_addr       : in  std_logic_vector(11 downto 0);    -- Endere�o a ser acessado (tanto pra escrita do STR quanto pra leitura do LOAD)
...
architecture Behavioral of memory is
    ...
    constant mem_size : integer := 4096; -- Tamanho da mem�ria (4096 linhas)
    subtype palavra is std_logic_vector(31 downto 0); -- Define que a palavra tem 32 bits
    type memory is array (0 to mem_size-1) of palavra; -- Define que a mem�ria ter� 4096 linhas com 32 bits cada (tamanho da palavra)
    ...
    signal instr_mem : memory := (
        ...
        -- "00100010000100001XXXXXXXXXXXXXXX", -- (5) 
        -- "00101010100100001XXXXXXXXXXXXXXX", -- (6)  
        -- "00001XXXX00100001XXX000000010100", -- (7)
        -- "00001XXXX00000011XXX000000010100", -- (8) 
        ...
        -- "00000000000000000000000000000000",
        -- "00000000000000000000000000000000",
        -- "000100110XXXXXXXXXXX000000000100", -- (20) 
        -- "00011XXXX0110XXXXXXX000000000101", -- (21)

        -- Programa do trablaho 2:
        "000100001XXXXXXXXXXX000000000001", -- (1)
        "000100010XXXXXXXXXXX000000000010", -- (2)
        -- "01000001100010010XXXXXXXXXXXXXXX", -- (3)
        "01001010000010010XXX000000000101", -- (4)
...
begin 
    ...
    begin
        if rising_edge(clk) then -- So reseta, escreve ou l� em uma mudan�a de clock
